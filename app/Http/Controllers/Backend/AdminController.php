...
class AdminController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $current_location_id = $request->location_id ? $request->location_id : $locations[0]->id;
    ...
    public function editOrder(Request $request) {
        ...
        $location_vehicles = LocationVehicles::leftJoin('vehicles', 'vehicles.id', '=', 'location_vehicles.vehicle_id')->where("location_id", $order->location_id)->get();
        $location_services = LocationServices::leftJoin('services', 'services.id', '=', 'location_services.service_id')->where("location_id", $order->location_id)->get();
        $location_pesuboxs = LocationPesuboxs::where("location_id", $order->location_id)->where("is_delete", 'N')->where("status", 1)->get();
        return view('backend.home.components.modal', compact("order", "id", "location_vehicles", "location_services", "location_pesuboxs"))->render();
    }
